...
if test -e /etc/profile.local; then
  echo "CV4_HOME=$INSTALLDIR/cv4linux" >> /etc/bash.bashrc
  echo "export CV4_HOME" >> /etc/bash.bashrc
  echo "PATH=$PATH:$INSTALLDIR/cv4linux" >> /etc/bash.bashrc
...
echo " This installation package will only work for SLES $TARGET. "
echo
echo 
...
echo
echo "** D O W N L O A D **"
echo "2.  Attempting to download neccessary package..."
wget http://www.aesclever.com/aftp/.linux3.0/RPM/SLES/$TARGET/$FILENAME
sleep 1
# check error 
if [ $? -ne 0 ]; then
    echo "Error downloading neccessary package.  Script terminated."
    exit 1
fi
echo
# check error 
if [ -e $FILENAME ]; then
    ...
    echo "software on your `uname -n`  `lsb_release -d`."
    echo
else
    echo "Error installing the package.  Script terminated."
    echo "Expected install package ($FILENAME) is not found."
    echo "Setup aborted."
    exit 0
fi
echo
...
echo -n "3. Verifying for system compatibility... "
lsb_release -d
if [ $? -ne 0 ]; then
   uname -a
fi
echo
echo -n "Preserve existing license and external configuration if any... "
...
echo "Done."
echo -n "Attempting to uninstall cv4tcpipl package, if any...:"
rpm -e cv4tcpipl >> /dev/null 2>&1
sleep 1
echo "... Done."
echo "Installing new cv4tcpipl package:"
DVER=`getdversion`
case "$DVER" in
    "rhel60"|"rhel61"|"rhel63"|"rhel64")
        ...
        INSTALLDIR="/usr/share"
        echo "Sorry.  Due to environment restriction on x86_64 $DVER, package relocation is not possible."
...
echo "4.  Configuring cv4tcpipl..."
PATH=$PATH:$INSTALLDIR/cv4linux
export PATH
sync
...
sh $INSTALLDIR/cv4linux/configure.sh
cd $CURRDIR
...
echo
echo "Note, your software package has been installed onto $(cat /etc/cv4env.conf)"
echo
